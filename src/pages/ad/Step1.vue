          ...
          <div class="input-container">
            ...
            <div class="select-container">
              ...
              <div class="select-list" v-if="this.categoryYn && this.type === 1">
                <p v-for="category in this.categoryList"
                   @click="this.categoryYn = false;
                            this.categoryName = category.name;
                            this.category = category.category;"
                >{{ category.name }}</p>
              </div>
              <div class="select-list" v-if="this.categoryYn && this.type === 2">
                <p v-for="category in this.serviceCategoryList"
                   @click="this.categoryYn = false;
                            this.categoryName = category.name;
                            this.category = category.category;"
                >{{ category.name }}</p>
            ...
            <p class="label">{{ this.type === 1 ? '5. 제품 링크':'4. 연결 링크' }} <span class="tip">제품 가격의 ‘최종 판매 가격’의 가격과 동일한 제품 링크를 입력해 주세요!</span></p>
            ...
            <div class="keyword-list">
              <div class="keyword" v-for="(k,i) in keywordsList">#{{k.value}} <span @click="removeKeyword(i)">x</span></div>
            </div>
            <p class="label">{{ this.type === 1 ? '7. 홍보 이미지':'6. 홍보 이미지' }}</p>
            <label class="chk-txt"><input type="checkbox" v-model="this.image_support"> 이미지 제작 지원이 필요하신 분은 체크해 주세요. (사이즈 조절 범위, 디자인 지원)</label>
            <div class="img-upload-container">
              ...
              <div class="upload-area">
                <input type="file" ref="images_new" v-on:change="handleFilesUpload()" class="hidden" style="display: none;" accept="image/*"/>
                <div class="upload-wrap" @click="addFiles">
                  <img class="" src="/image/ad/img-upload.png" v-if="this.images_new.length === 0 && this.images_view.length === 0">
                  <img class="" :src="this.images_new[0].preview" v-if="this.images_new.length !== 0 && this.images_view.length === 0">
